...
## Den číslo
table_rows = page_soup.find("table", {"id": "program_data"}).findAll("tr", recursive=False)
...
## Divadlo
      theater = container.findAll("div", {"class":"item-building"})[0].text.split(",")[0].split("-")[0].strip()
      if theater == "Malá scéna Mahenova divadla":
          theater = "Mahenovo divadlo"

## Čas
      time_container = container.findAll("div",{"class":"item-hours"})[0].text.split("-")[0].strip()
      time = str(time_container) +":00"

## Název hry
      title = container.findAll("div",{"class":"item-title"})[0].text.strip()
